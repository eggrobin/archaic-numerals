...
% \begin{itemize}[nosep]
%   \item U+\textcsc{12550}--U+\textcsc{12558} \oneAšC--\nineAšC \textcsc{CUNEIFORM NUMERIC SIGN ONE N01--CUNEIFORM NUMERIC SIGN NINE N01}
%   \item U+\textcsc{1257D}--U+\textcsc{12585} \oneDišC--{TODO} \textcsc{CUNEIFORM NUMERIC SIGN ONE N08--CUNEIFORM NUMERIC SIGN NINE N08}
...
%  \item \cite[\reverse~1, 1]{P221305} {\curviform\oneŊešTwoC\fourUC\oneBanTwoC\xsuxfont 𒂠𒑍𒄀} % 1(gesz2@c) 4(u@c) 1/2(asz@c) esz2 5(disz@t) gi
  ...
  \item \cite[\reverse~2, 1]{P221266} {\oneAšC\curviform\oneBanTwoC\xsuxfont 𒂠𒑋𒄀} % 1(asz@c) 1/2(asz@c) esz2 3(disz@t) gi
\end{itemize}
...
\end{enumerate}
  %\item \cite[\obverse iii 2]{P240964} {\sixAšC\xsuxfont 𒇷\curviform\sixŊešTwoC\oneDišC\xsuxfont 𒄘𒁇\rotatebox[origin=c]{270}{\twoAšC}\xsuxfont 𒋙𒃻𒈫𒀭𒍡} [TODO comment or stick it in its own paragraph] % EbDA http://ebda.cnr.it/tablet/view/3184#155353, EST 061.
...
{\oneAšC\xsuxfont 𒈪𒀜\curviform\oneŊešTwoC\threeUC\fiveAšC\xsuxfont 𒊺 𒄘𒁇}
...
\vphantom{\scalebox{0.85}{\sixŊešTwoC}}{\smash{\sixŊešTwoC}\threeUC\xsuxfont 𒃻𒁺\curviform\oneBanTwoC\xsuxfont 𒑌𒄀}
...
is highly inconvenient, as it requires
...
distinguish some stacking patterns;
...
besides {\xsuxfont 𒑖} and {\xsuxfont 𒑗} which are characteristic of bariga measures,
...
This is also for compatibility with distinctions made in
reference works and in some non-numeric transliterations;
for instance, {\xsuxfont 𒐼} is \cite[No.~860]{Borger2010} and has the value limmu,
and {\xsuxfont 𒐉} is \cite[No.~852]{Borger2010} and  has the value limmu₅.

However, there is no such practice when it comes to numeric transliterations\footnote{The Sumerian word
...
The stacking patterns used in numeric expressions change over time,
and do not carry any semantics; the typical $4(\text{diš})$ is {\xsuxfont 𒐉} in Ur III,
but {\xsuxfont 𒐼} in the Neo-Assyrian period.

Since they are not listed separately in classical sign lists nor
assigned any different values, the stacking patterns from earlier periods
are not separately encoded; for instance,
in ED IIIb Ŋirsu, {\xsuxfont 𒎙} $2$(u) often has one {\xsuxfont 𒌋} atop another.
There is no evidence of a need to encode such variant stacking patterns;
...
this is illustrated in Figure~\ref{twentyTwoOxen}.
Note that in that figure,
the numerals need to be considerably enlarged in order to reproduce the layout of the
tablet: \oneAšC{} has the same height as {\xsuxfont 𒄞}.
...
typesetting these numerals, as illustrated in section~\ref{modernUsage}:
reproducing the layout of tablets is not within the scope of plain text.
\begin{figure}
\newlength{\twentyTwoOxenWidth}
\settowidth{\twentyTwoOxenWidth}{{\LARGE\twoAšC}{\xsuxfont 𒄞}}
\begin{center}
  \rotatebox[origin=c]{270}{\LARGE\twoUC}\parbox{\twentyTwoOxenWidth}{%
  \linespread{0}\selectfont{}\vphantom{\scalebox{0.6}{\LARGE\twoAšC}}\smash{{\LARGE\twoAšC}\raisebox{2pt}{\xsuxfont 𒄞}}\\
  \vphantom{\scalebox{0.6}{\LARGE\twoAšC}}{\hspace{0.5em}\smash{\xsuxfont 𒈬}\hspace{0.5em} \smash{\xsuxfont 𒀹}\hfill}}
...
\]
  ...
  \cites[\reverse]{P001243}[\reverse~2]{P004500}. All the Uruk examples are transliterated
  $9$(N34) in CDLI, and the later ones $9$(gesz2@c).}
...
and Figure~\ref{twentyTwoOxen} rotates the character \twoUC{},
