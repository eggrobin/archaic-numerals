...
\newcommand\oneNTwentyFive{{\proposalfont\symbol{"125AB}}}
...
\multicolumn{4}{l}{tugₓ(LAK483)-si-ga-kam}\\
tugsiga&=ak&=am&-∅\\
...
\cite{Feuerherm2004} discussed in section~\ref{transliteration}.
...
\begin{equation}
  ...
  \xleftarrow{30}\underset{\substack{\text{UŠ\footnotemark}\\\text{cable}\\360~\text{m}}}{\text{\xsuxfont 𒁹𒍑}}
  \xleftarrow{60}\underset{\substack{\text{nindan}\\\text{\emph{nindanum}}\\\text{rod}\\6~\text{m}}}{\text{\xsuxfont 𒁹𒃻}}
...
and the fractions $\frac13$ and $\frac23$ are written {\xsuxfont 𒑚} and
{\xsuxfont 𒑛}.
...
(and earlier in the fourth millenium) clearly mirrors system \ref{systemSOB}
...
of one iku and greater mirrors system \ref{systemGOB}
...
\tag{$G_{\text{ED IIIb}}$}
\end{equation*}
with consistent use of the numerals: \oneUC{} corresponds to {\xsuxfont 𒌋},
\oneŠarTwoC{} to {\xsuxfont 𒊹}, and \oneŠarʾuC{} to {\xsuxfont 𒐬}.
An exception to this correspondence, noted in \cite[4]{L2/04-099} (see section~\ref{oldArgumentsForUnification}),
...
System \ref{systemG} is used in the fourth millenium, but also
...
system \ref{systemGED}, with curviform numerals\footnote{A CDLI search for \texttt{"(bur3)"} ({\xsuxfont 𒌋} numerals used for areas)
...
These can be summarized by the following factor diagram:
\begin{equation}
  \underbrace{
  \text{\oneŊešʾuC}
  \xleftarrow{10}\text{\oneŊešTwoC}
...
\cite[\obverse~2, 1]{P020129} {\oneŊešTwoC\oneUC\xsuxfont 𒑋𒄀}.}
...
\xleftarrow{6}{\text{\xsuxfont 𒀹𒋡}}\text,
\tag{$C_{\text{\xsuxfont 𒄥𒊕𒅅}}$}
...
the single occurrence of (ban2@c) followed by curviform numerals and sila3 in ED IIIb Ŋirsu transliterations on CDLI,
...
{\xsuxfont\twoAšC 𒂁𒇲𒑞} ``two pots minus one third (sila₃)'',
...
We shall not discuss it further, as presents no complexity for character encoding.
It is noteworthy that the sign {\xsuxfont 𒇲} (lal₂, ``minus'')
...
is outside the realm of plain text; see also section~\ref{stackingPatterns}.} for both the {\xsuxfont 𒂁} and the {\xsuxfont 𒋡}, thus \cite[504\psq]{Powell1987}\begin{equation}
...
Another capacity system in ED IIIb Ŋirsu is the {\xsuxfont 𒄥𒑊𒌌}, the gur of two ul:
\begin{equation}
\underbrace{\text{\oneUC}
\xleftarrow{10}{\text{\oneAšC}}}_{\text{\xsuxfont 𒄥𒑊𒌌}}
\xleftarrow{2}{\text{\oneDišC}}
\xleftarrow{6}{\text{\oneBanTwoC}}
\xleftarrow{6}{\text{\xsuxfont 𒀹𒋡}}\text.
...
Some instances of that usage are found transliterated $n/6$ in EbDA; in some cases the {\xsuxfont 𒂆} sign is omitted, and
...
\begin{enumerate}[nosep]
  ...
  \item \cite[\href{http://ebda.cnr.it/tablet/view/3184\#155353}{\recto~3, 2}]{P240964}\footnote{From
  photo in \cite[6]{Archi1989}; see also the CDLI photo and the copy in \cite[17]{Friberg1986}.
  ...
  counted with cuneiform numerals, and the higher units with cuneiform numerals.} {\xsuxfont\sixAšC 𒇷\footnote{Short for {\xsuxfont 𒇷𒅎}.} \oneŊešTwoC\oneDišC 𒄘𒁇 𒈫𒉻𒊕\footnote{ŠU₂+NIG₂-sag, an unusual variant spelling.} 𒈫𒀭𒍡}
\end{enumerate}
...
\begin{tikzcd}[column sep=small]
  \smash{\underbrace{\text{\oneAšC\xsuxfont 𒇷𒅎}
  \xleftarrow{10}\text{\oneAšC\xsuxfont 𒈪𒀜}
  \xleftarrow{\frac{5}{3}}\text{\oneŊešTwoC}
  \xleftarrow{6}\text{\oneUC}
...
\label{modernUsage}
Because of their prevalence in the fourth millenium and Early Dynastic period,
...
\footnotetext{
  ...
  copies of fourth millenium and sometimes third millenium tablets are shown as vertical text
  ...
  \begin{align}
    ...
    \tag{\text{C 314}}\\
    1\text{\oneNFiftyFourC}+1\text{\oneNFiftyOneC}+1\text{\oneŊešTwoC}&=1\text{\oneNFiftyOneC}+2\text{\oneŊešTwoC}+6\text{\oneUC}\text.
    \tag{\text{C 27}}
  \end{align}
...
\begin{figure}[H]
  ...
  \caption[]{Description of the fractions \oneNTwentyFour{} and \oneNTwentyFive{} in \cite[113]{Englund1998}\footnotemark.\label{englund113}}
  \end{center}
\end{figure}
...
\begin{figure}[H]
  ...
  \end{center}
\end{figure}
\footnotetext{Compare {\xsuxfont 𒊹𒃲} discussed in section~\ref{areaSystem}.
The order is variable in the Early Dynastic period; see \cite{P010773}, also discussed in \cite[148]{Friberg2007},
for an example of {\xsuxfont 𒃲\oneŠarTwoC},
and \cites{P274845}{P241764} for examples of {\xsuxfont 𒃲$n$\oneŠarTwoC}.
Sign order can be variable in early texts, see \cite[8]{Foxvog2016}.}
\begin{figure}[H]
  \begin{center}
...
in section~\ref{šar₂}, and
{\xsuxfont 𒋢𒆳𒊒𒆠𒂊 𒌉𒉌𒊏 𒈾 𒈾𒈬𒌦𒊑𒊑}
in the OB composite \cite[6]{Q000782} (translated ``Šuruppag gave instructions to his son''
in \cite{ETCSRI}).
...
mentioned in section~\ref{capacity}, these (already-encoded) characters were clearly
...
periods\footnote{See, \exempligratia, \cite[55]{Robson2008} on the unification of
metrologies under Sargon, resulting in the systems described in
section~\ref{metrology}.}, there is little opportunity
...
is transliterated ``$1$(eše₃) $5\text{½}$~iku\footnote{{\xsuxfont 𒃷} interpreted as a unit, as discussed in section~\ref{metrology}.} $7$~sar'' in \volcite{2}[176]{Feuerherm2004},
...
A designer concerned about the numeric-syllabic \oneAšC--{\xsuxfont 𒀸},
...
but this would break capacity measures that use \oneDišC{};
in an effort to support diachronic use for $1/2$(iku),
...
an ED I--II Ur font could decide to give {\xsuxfont 𒐴}
the same glyph as {\xsuxfont 𒐬} (that of the proposed \oneŠarʾuC), according to the older area system,
making it impossible to represent the newer one.
...
but that are also attested in the fourth millenium in the Uruk III period;
the fourth millenium, especially the Uruk IV period, also frequently features numerals that use a more vertical layout,
